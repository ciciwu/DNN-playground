...
class NeuralNet2(nn.Module):
    def __init__(self, input_size, num_classes):
        ...
        self.linear1 = nn.Linear(input_size, 35) 
        self.relu = nn.ReLU()
...
num_epochs = 10
optimizer = torch.optim.SGD(model.parameters(), lr=0.0000001,weight_decay=0.5) 
...
for epoch in range(num_epochs):
    for i, (features, labels) in enumerate(train_loader):  
        ...
        # if (i+1) % 2 == 0:
        print (f'Epoch [{epoch+1}/{num_epochs}], Step [{i+1}], Loss: {loss.item():.12f}')
